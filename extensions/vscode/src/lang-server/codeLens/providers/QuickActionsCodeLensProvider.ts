import { DebuggAiConfig, QuickActionConfig } from "core";
...
export function getQuickActionsConfig(config: DebuggAiConfig) {
...
export class QuickActionsCodeLensProvider implements vscode.CodeLensProvider {
  ...
  getCustomCommands(
    ...
    return quickActionConfigs.map(({ title, prompt, sendToChat }) => {
      return sendToChat
        ? {
            title,
            command: "debugg-ai.customQuickActionSendToChat",
            arguments: [prompt, range],
          }
        : {
            title,
            command: "debugg-ai.customQuickActionStreamInlineEdit",
            arguments: [prompt, range],
          };
    });
  }
  ...
  getDefaultCommand(range: vscode.Range): vscode.Command[] {
    const quickEdit: vscode.Command = {
      command: "debugg-ai.defaultQuickAction",
      title: "Continue",
