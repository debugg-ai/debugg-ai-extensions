...
Continue respects `.gitignore` files in order to determine which files should not be indexed. If you'd like to exclude additional files, you can add them to a `.debuggignore` file, which follows the exact same rules as `.gitignore`.

Continue also supports a **global** `.debuggignore` file that will be respected for all workspaces, which can be created at `~/.continue/.debuggignore`.
