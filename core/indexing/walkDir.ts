...
export async function getIgnoreContext(
  ...
  // These are done separately so that .debuggignore can override .gitignore
  const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore");
  const continueIgnoreFile = dirFiles.find(
    (name) => name === ".debuggignore",
  );
  ...
  const getContinueIgnorePatterns = async () => {
    if (continueIgnoreFile) {
      const contents = await ide.readFile(`${currentDir}/.debuggignore`);
      return gitIgArrayFromFile(contents);
    }
    return [];
  };
  ...
  const ignoreContext = ignore()
    ...
    .add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .debuggignore - this is combined for speed
    .add(ignoreArrays[1]); // local .debuggignore
