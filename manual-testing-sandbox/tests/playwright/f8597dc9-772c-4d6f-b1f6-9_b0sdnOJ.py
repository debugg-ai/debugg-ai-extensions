...
async def run_generated_script():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            context = await browser.new_context(permissions=['clipboard-read', 'clipboard-write'], record_video_dir='/app/backend/media/tmptksyf2qx.gif', no_viewport=True)
            ...
            final_message = replace_sensitive_data("TUNNELING ERROR - The ngrok endpoint is offline and the target page cannot be accessed to verify the hero section as requested. The task cannot be completed in the current state.", SENSITIVE_DATA)
            print(final_message)
