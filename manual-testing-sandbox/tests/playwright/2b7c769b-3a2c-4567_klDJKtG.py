...
async def run_generated_script():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            context = await browser.new_context(permissions=['clipboard-read', 'clipboard-write'], record_video_dir='/app/backend/media/tmp6e0u_et1.gif', no_viewport=True)
            ...
            final_message = replace_sensitive_data("TUNNELING ERROR: The ngrok endpoint is offline and the page content cannot be accessed. The task cannot be completed.", SENSITIVE_DATA)
            print(final_message)
